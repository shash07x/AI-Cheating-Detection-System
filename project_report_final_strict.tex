...
The remaining chapters are organized according to the required academic structure. Chapter 2 presents the literature survey for the project domain. Chapter 3 explains the overall system overview and high-level architecture. Chapter 4 summarizes the technologies used in the repository. Chapter 5 describes the system design and data flow logic. Chapter 6 covers implementation details with module-wise and file-wise explanation. Chapter 7 presents results and analysis. Chapter 8 focuses on testing. Chapter 9 discusses strengths, limitations, and practical issues. Chapter 10 concludes the report and outlines future work.
...
\section{Introduction}
This chapter presents the overall system overview of the repository. It explains the operational architecture already present in code, the current and proposed workflow, and the requirements that shape the remote interview monitoring platform.
...
\section{Introduction}
The repository uses a broad but coherent technology stack centered on Python for backend intelligence and JavaScript with React for user-facing applications. The selected tools indicate a strong focus on real-time communication, practical multimedia analysis, and rapid prototyping of AI-assisted proctoring features.

\section{Programming Languages}
\begin{table}[H]
\centering
\caption{Primary programming languages found in the repository}
\begin{tabularx}{\textwidth}{p{3cm}X}
\toprule
\textbf{Language} & \textbf{Observed Use in the Repository}\\
\midrule
Python & Backend services, Flask routes, socket handlers, computer-vision logic, audio analysis, model loading, testing, and utility scripts\\
JavaScript & Candidate application, interviewer dashboard, socket clients, chart rendering, and frontend API integration\\
HTML/CSS & React application templates, styling files, and browser-facing layout support\\
\bottomrule
\end{tabularx}
\end{table}

\section{Backend Libraries and Frameworks}
The backend dependency file \texttt{backend/requirements.txt} shows the practical implementation stack used by the project. Flask provides the web framework, Flask-CORS supports cross-origin communication, Flask-SocketIO handles real-time bidirectional events, and Flask-SQLAlchemy appears in support of structured persistence. NumPy and SciPy support numerical processing, while OpenCV is used for image decoding and frame analysis. Ultralytics YOLO, PyTorch, and TorchVision support the visual-detection pipeline. librosa and soundfile support audio loading and feature extraction. scikit-learn appears in the environment for classical machine-learning utilities. The dependency list also includes \texttt{google-generativeai}, indicating experimentation with language-oriented scoring or AI-assisted text analysis.

\begin{table}[H]
\centering
\caption{Key backend technologies identified from the repository}
\begin{tabularx}{\textwidth}{p{3.5cm}X}
\toprule
\textbf{Technology} & \textbf{Role in the System}\\
\midrule
Flask & REST API framework and application factory backbone\\
Flask-SocketIO & Real-time streaming, live risk updates, and session events\\
OpenCV & Frame decoding, preprocessing, and image manipulation\\
Ultralytics YOLO & Person and mobile-phone detection in video frames\\
PyTorch / TorchVision & Model execution and compatibility for detection modules\\
librosa / soundfile & Audio loading, preprocessing, silence handling, and feature extraction\\
NumPy / SciPy & Numeric processing for image and audio pipelines\\
scikit-learn & Supporting machine-learning utilities and classical processing steps\\
google-generativeai & AI text-related experimentation or service integration\\
\bottomrule
\end{tabularx}
\end{table}

\section{Frontend Libraries and Frameworks}
The candidate-side and interviewer-side package manifests confirm the use of React 18, React DOM, React Scripts, Socket.IO client, and testing-library packages. The interviewer dashboard also includes Axios for HTTP communication, Recharts and Chart.js for visualization, React ChartJS 2 for chart binding, and \texttt{react-gauge-chart} for risk gauges. This stack is appropriate for a dashboard-centric application where live updates, interactive metrics, and clear visual status presentation are important.

\begin{table}[H]
\centering
\caption{Key frontend technologies identified from package manifests}
\begin{tabularx}{\textwidth}{p{3.4cm}X}
\toprule
\textbf{Technology} & \textbf{Role in the System}\\
\midrule
React / React DOM & Candidate and interviewer single-page interfaces\\
React Scripts & Build, development, and test lifecycle under Create React App\\
Socket.IO Client & Live communication with backend socket handlers\\
Axios & HTTP request handling for dashboard interactions\\
Chart.js / React ChartJS 2 & Graph-based visualization of session metrics\\
Recharts & Dashboard charts such as timelines and score trends\\
React Gauge Chart & Fraud and risk gauge visualization\\
Testing Library & Frontend unit and interaction testing support\\
\bottomrule
\end{tabularx}
\end{table}

\section{Models, Data Assets, and Supporting Tools}
The repository also contains pretrained and supporting assets such as \texttt{yolov8n.pt}, \texttt{yolov8n-pose.pt}, FAISS and pickle files for text-analysis support, TensorFlow or Keras-based voice-model artifacts, and the ASVSpoof2019 dataset folder under \texttt{backend/ml/datasets}. These files show that the project combines classic software engineering with model-centric experimentation. Evidence-image folders, runtime logs, and audio log artifacts further indicate that the system was designed to observe real sessions rather than only static samples.

\section{Development Tools}
From the file structure, the main development tools include Python virtual environments, npm-based frontend package management, local log generation, test scripts, and markdown-based implementation notes. Together these tools support iterative development, debugging, and repository-level documentation.
...
The repository reveals several practical challenges that commonly arise in multi-modal monitoring systems. The first challenge is synchronizing heterogeneous streams. Video frames, audio chunks, tab events, and session commands do not arrive with identical timing or reliability. The code therefore uses route-level validation, socket event handlers, buffering logic, and stateful aggregation to keep the session coherent. The second challenge is balancing model-driven inference with simple heuristics. Some modules already use YOLO or richer feature extraction, while others deliberately fall back to lightweight rules when the main detector fails or when full models are not available.

A third challenge is version management. The simultaneous presence of \texttt{backend} and \texttt{backend\_v2}, along with both original and V2 frontends, shows that the system evolved through experimentation. This is useful from a learning standpoint, but it complicates maintenance, documentation, and deployment. A fourth challenge is interpretability. A proctoring system cannot rely on opaque numbers alone. The repository addresses this by pairing score bands with reasons, evidence images, alerts, and final report narratives, but the challenge of making those explanations consistent across all modules still remains.

\section{Solutions Reflected in the Repository}
The source code already reflects several solutions to the above challenges. Session-centric state management provides continuity across events. Evidence capture gives visual support for high-risk claims. Socket-based updates keep the interviewer informed during the session rather than only at the end. Fallback detectors increase resilience when richer pipelines fail. Modular service separation reduces coupling between the Flask app, detectors, reporting logic, and UI code.

The project also addresses operational complexity by splitting user roles across dedicated applications. The candidate application focuses on low-friction media capture and tab tracking, whereas the dashboard prioritizes explanation, scoring, and evidence review. This separation improves usability and aligns with the repository's event-driven architecture. Another practical solution is the presence of test scripts and implementation notes, which help developers understand edge cases and previously solved problems.

\section{Strengths of the Current Design}
The system demonstrates several design strengths:
\begin{enumerate}
  \item It treats cheating detection as a multi-modal problem rather than a single-detector task.
  \item It combines live monitoring with session-end report generation.
  \item It preserves evidence rather than depending only on transient scores.
  \item It keeps the interviewer in the loop through dashboard alerts and explanations.
  \item It shows iterative engineering maturity through modularization and version refinement.
\end{enumerate}

\section{Limitations and Risks}
The current design also has visible limitations. Some scoring policies remain heuristic. Some modules overlap in responsibility. Formal calibration and benchmark evaluation are not yet standardized inside the repository. Persistence paths appear uneven across versions. In real deployment, false positives, network instability, and fairness concerns would need deeper treatment than can be inferred from the current code alone.

\section{Interpretive Summary}
Overall, the discussion suggests that the repository is a strong applied-computing project with realistic implementation depth. Its main challenges are not a lack of ideas but the typical next-step engineering tasks of consolidation, calibration, documentation, and operational hardening.
...
\begin{center}
\bfseries IMPORTANT CODE LISTINGS AND SUPPORTING MATERIAL
\end{center}
This appendix includes representative source files and supporting figures from the current repository. The goal is to preserve the long project-report style of the sample file while grounding the appendix in real code already present in the workspace.

\codeannexsection[language=Python]{A.I.1 Backend Entry Point}{code/backend_run.py}
\codeannexsection[language=Python]{A.I.2 Main Video Route}{code/backend_app_routes_video_routes.py}
\codeannexsection[language=Python]{A.I.3 Audio Socket Handler}{code/backend_app_socket_handlers_audio_socket.py}
\codeannexsection[language=Python]{A.I.4 Stable Vision Engine}{code/backend_app_services_stable_vision_engine.py}
\codeannexsection[language=JavaScript]{A.I.5 Candidate Application}{code/frontend_candidate_app_src_CandidateApp.jsx}
\codeannexsection[language=JavaScript]{A.I.6 Interviewer Dashboard}{code/frontend_interviewer_dashboard_src_components_Dashboard.jsx}
\codeannexsection[language=Python]{A.I.7 Audio Routes}{code/backend_app_routes_audio_routes.py}
\codeannexsection[language=Python]{A.I.8 Application Factory}{code/backend_app___init__.py}
\codeannexsection[language=Python]{A.I.9 Fusion State Service}{code/backend_app_services_fusion_state.py}
\codeannexsection[language=Python]{A.I.10 Enhanced Audio Engine}{code/backend_app_services_enhanced_audio_engine.py}
\codeannexsection[language=Python]{A.I.11 Head Pose Estimation}{code/backend_app_services_head_pose_estimation.py}
\codeannexsection[language=Python]{A.I.12 Video Socket Handler}{code/backend_app_socket_handlers_video_socket.py}
\codeannexsection[language=Python]{A.I.13 Session Socket Handler}{code/backend_app_socket_handlers_session_socket.py}
\codeannexsection[language=JavaScript]{A.I.14 Webcam Sender}{code/frontend_candidate_app_src_WebcamSender.jsx}
\codeannexsection[language=JavaScript]{A.I.15 Microphone Sender}{code/frontend_candidate_app_src_MicrophoneSender.jsx}
\codeannexsection[language=JavaScript]{A.I.16 Interview Dashboard Page}{code/frontend_interviewer_dashboard_src_pages_Dashboard.jsx}
\codeannexsection[language=JavaScript]{A.I.17 Final Report Component}{code/frontend_interviewer_dashboard_src_components_FinalReport.jsx}
\codeannexsection[language=Python]{A.I.18 Vision Service V2}{code/backend_v2_app_services_vision_service_v2.py}
\codeannexsection[language=Python]{A.I.19 Audio AI V2}{code/backend_v2_app_services_audio_ai_v2.py}
...
\begin{figure}[H]
\centering
\safeimage[width=0.82\textwidth]{images/backend_architecture.png}
...
\begin{figure}[H]
\centering
\safeimage[width=0.82\textwidth]{images/candidate_interface.png}
...
\begin{figure}[H]
\centering
\safeimage[width=0.82\textwidth]{images/interviewer_dashboard.png}
...
\begin{figure}[H]
\centering
\safeimage[width=0.78\textwidth]{images/evidence_samples.png}
...
\begin{figure}[H]
\centering
\safeimage[width=0.78\textwidth]{images/session_timeline.png}
...
\chapter*{ANNEXURE V}
\addcontentsline{toc}{chapter}{ANNEXURE V - ADDITIONAL BACKEND LISTINGS}
\begin{center}
\bfseries ADDITIONAL BACKEND LISTINGS
\end{center}
This annexure extends the source archive with additional backend services that influence score generation, report preparation, and audio interpretation. Including these files increases the documentary completeness of the report and ensures that the implementation narrative is backed by representative source material from multiple subsystems.

\codeannexsection[language=Python]{A.V.1 Fraud Aggregator}{code/backend_app_services_fraud_aggregator.py}
\codeannexsection[language=Python]{A.V.2 Final Report Engine}{code/backend_app_services_final_report_engine.py}
\codeannexsection[language=Python]{A.V.3 Audio Analysis Service}{code/backend_app_services_audio_analysis.py}
\codeannexsection[language=Python]{A.V.4 Audio AI Heuristics}{code/backend_app_services_audio_ai.py}

\chapter*{ANNEXURE VI}
\addcontentsline{toc}{chapter}{ANNEXURE VI - SUPPLEMENTARY FRONTEND AND V2 LISTINGS}
\begin{center}
\bfseries SUPPLEMENTARY FRONTEND AND V2 LISTINGS
\end{center}
This annexure preserves additional user-interface and alternate-version source files from the repository. These listings are useful for understanding how the candidate experience, dashboard composition, and simplified V2 backend evolved during the project lifecycle.

\codeannexsection[language=JavaScript]{A.VI.1 Tab Tracker}{code/frontend_candidate_app_src_TabTracker.jsx}
\codeannexsection[language=JavaScript]{A.VI.2 Fraud Alerts Component}{code/frontend_interviewer_dashboard_src_components_FraudAlerts.jsx}
\codeannexsection[language=JavaScript]{A.VI.3 Risk Timeline Graph Component}{code/frontend_interviewer_dashboard_src_components_RiskTimelineGraph.jsx}
\codeannexsection[language=Python]{A.VI.4 V2 Video Routes}{code/backend_v2_app_routes_video_routes.py}
